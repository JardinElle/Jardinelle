...
const FLOWERS = [
  {
    ...
    colors: [
      { name: "Rouge Passion",  hex: "#c0392b" },
      { name: "Rose Pastel",    hex: "#f48fb1" },
  ...
  },
  {
    ...
    colors: [
      ...
      { name: "Rose Pastel",    hex: "#ffd6e0" },
      { name: "Pêche",          hex: "#ffcba4" },
      { name: "Jaune",          hex: "#fff176" },
      { name: "Orange",         hex: "#ff7043" },
      { name: "Violet",         hex: "#ce93d8" },
      { name: "Bleu Ciel",      hex: "#81d4fa" },
...
// ─── MULTI-VARIANT FLOWER PICKER ─────────────────────────────────────────────
// flowerVariants = [{ flowerID, colorHex, qty }]

function FlowerVariantPicker({ flowers, variants = [], onChange }) {
  const [pendingFlower, setPendingFlower] = useState(null);
  const [pendingColor,  setPendingColor]  = useState(null);
  const [pendingQty,    setPendingQty]    = useState(1);
  const [error, setError] = useState("");

  const selectedFlower = flowers.find(f => f.id === pendingFlower);
  const isLight = (h) => ["#ffffff","#fff176","#ffd6e0","#ffcba4","#81d4fa","#d8b4e2","#ce93d8","#fff8e1","#fafafa"].includes(h);

  const handleSelectFlower = (id) => {
    setPendingFlower(id);
    const f = flowers.find(fl => fl.id === id);
    setPendingColor(f?.colors[0].hex || null);
    setPendingQty(1);
    setError("");
  };

  const handleAdd = () => {
    if (!pendingFlower) { setError("Choisissez une fleur."); return; }
    if (!pendingColor)  { setError("Choisissez une couleur."); return; }
    const updated = [...variants, { flowerID: pendingFlower, colorHex: pendingColor, qty: pendingQty }];
    onChange(updated);
  ...
  const handleVariantQty = (idx, delta) => {
    const updated = variants.map((v, i) => i === idx ? { ...v, qty: Math.max(1, v.qty + delta) } : v);
    onChange(updated);
  };

  const totalAdded = variants.reduce((s, v) => s + v.qty, 0);

  return (
    <div className="fvp-wrapper">
      {/* ── Added variants list ── */}
      {variants.length > 0 && (
        <div className="fvp-list">
          <div className="fvp-list-title">🌷 Composition ({totalAdded} fleur{totalAdded > 1 ? "s" : ""})</div>
          {variants.map((v, i) => {
            const f  = flowers.find(fl => fl.id === v.flowerID);
            const cn = f?.colors.find(c => c.hex === v.colorHex)?.name || "";
            return (
              <div key={i} className="fvp-variant-row">
                <span className="fvp-dot" style={{background: v.colorHex, border: isLight(v.colorHex) ? "1px solid #ccc" : "none"}} />
                <span className="fvp-variant-name">{f?.emoji} {f?.name} · <em>{cn}</em></span>
                <div className="fvp-qty-row">
                  <button className="qty-btn" onClick={() => handleVariantQty(i, -1)} disabled={v.qty <= 1}>−</button>
                  <span className="qty-num">{v.qty}</span>
                  <button className="qty-btn" onClick={() => handleVariantQty(i, 1)}>+</button>
                </div>
                <span className="fvp-price">{fmtPrice(f ? f.price * v.qty : 0)}</span>
                <button className="fvp-remove" onClick={() => handleRemoveVariant(i)} title="Retirer">✕</button>
              </div>
            );
          })}
        </div>
      ...
      <div className="fvp-add-panel">
        <div className="fvp-panel-title">➕ Ajouter des fleurs</div>

        {/* Flower type selector */}
        <div className="fvp-flower-grid">
          {flowers.map(f => (
            <button key={f.id}
              className={`fvp-flower-btn${pendingFlower === f.id ? " selected" : ""}`}
              onClick={() => handleSelectFlower(f.id)}>
              <span className="fvp-flower-emoji">{f.emoji}</span>
              <span className="fvp-flower-label">{f.name}</span>
              <span className="fvp-flower-price">{fmtPrice(f.price)}/u</span>
            </button>
          ))}
        </div>

        {/* Color + qty (only when flower is selected) */}
        {selectedFlower && (
          <div className="fvp-color-block">
            <div className="fvp-color-label">🎨 Couleur — {selectedFlower.name}</div>
            <div className="fvp-color-row">
              {selectedFlower.colors.map(c => (
                <button key={c.hex}
                  className={`fvp-color-dot${pendingColor === c.hex ? " active" : ""}`}
                  style={{background: c.hex, border: isLight(c.hex) ? "1.5px solid #ccc" : "none"}}
            ...
            {pendingColor && (
              <div className="fvp-color-name-badge" style={{
                background: pendingColor,
                color: isLight(pendingColor) ? "#555" : "#fff",
                border: isLight(pendingColor) ? "1px solid #ccc" : "none"
              }}>
                {selectedFlower.colors.find(c => c.hex === pendingColor)?.name}
              </div>
            )}

            <div className="fvp-qty-block">
              <span className="fvp-qty-label">Quantité :</span>
              <button className="qty-btn" onClick={() => setPendingQty(q => Math.max(1, q - 1))} disabled={pendingQty <= 1}>−</button>
              <span className="qty-num fvp-qty-num">{pendingQty}</span>
              <button className="qty-btn" onClick={() => setPendingQty(q => q + 1)}>+</button>
              <span className="fvp-subtotal">{fmtPrice(selectedFlower.price * pendingQty)}</span>
            </div>

            {error && <p className="field-err-msg">{error}</p>}
            <button className="fvp-add-btn" onClick={handleAdd}>
              ✅ Ajouter {pendingQty} {selectedFlower.name}{pendingColor ? ` · ${selectedFlower.colors.find(c=>c.hex===pendingColor)?.name}` : ""}
            </button>
          </div>
        )}
      </div>
...
// ─── HEART VARIANT PICKER ────────────────────────────────────────────────────
// heartVariants = [{ colorID, colorHex, colorName, qty }]
function HeartVariantPicker({ variants = [], onChange }) {
  const [pendingColor, setPendingColor] = useState(null);
  const [pendingQty,   setPendingQty]   = useState(1);
  const isLight = (h) => ["#ffffff","#fff176","#ffcba4"].includes(h);
  const HEART_PRICE = 0; // coeur inclus dans le prix pot

  const handleAdd = () => {
    if (!pendingColor) return;
    const c = HEART_COLORS.find(h => h.id === pendingColor);
    const updated = [...variants, { colorID: pendingColor, colorHex: c.hex, colorName: c.name, qty: pendingQty }];
    onChange(updated);
    setPendingColor(null);
    setPendingQty(1);
  };
  ...
  const handleQty = (idx, delta) => onChange(variants.map((v, i) => i === idx ? { ...v, qty: Math.max(1, v.qty + delta) } : v));

  const totalHearts = variants.reduce((s, v) => s + v.qty, 0);

  return (
    <div className="fvp-wrapper" style={{marginTop: 12}}>
      {variants.length > 0 && (
        <div className="fvp-list">
          <div className="fvp-list-title">💗 Cœurs ({totalHearts} au total)</div>
          {variants.map((v, i) => (
            <div key={i} className="fvp-variant-row">
              <span className="fvp-dot" style={{background: v.colorHex, border: isLight(v.colorHex) ? "1px solid #ccc" : "none"}} />
              <span className="fvp-variant-name">💗 Cœur · <em>{v.colorName}</em></span>
              <div className="fvp-qty-row">
                <button className="qty-btn" onClick={() => handleQty(i, -1)} disabled={v.qty <= 1}>−</button>
                <span className="qty-num">{v.qty}</span>
                <button className="qty-btn" onClick={() => handleQty(i, 1)}>+</button>
              </div>
              <span className="fvp-price" style={{color:"var(--text-lt)",fontSize:".75rem"}}>inclus</span>
              <button className="fvp-remove" onClick={() => handleRemove(i)} title="Retirer">✕</button>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="fvp-add-panel">
        <div className="fvp-panel-title">➕ Ajouter des cœurs</div>
        <div className="fvp-color-label">🎨 Couleur du cœur</div>
        <div className="color-picker-row" style={{flexWrap:"wrap", gap:6}}>
          {HEART_COLORS.map(c => (
            <button key={c.id}
              className={`color-pill${pendingColor === c.id ? " active" : ""}`}
              onClick={() => setPendingColor(c.id)}>
              <span className="pill-swatch" style={{background: c.hex, border: isLight(c.hex) ? "1px solid #ddd" : "none"}} />
              <span className="pill-name">{c.name}</span>
            </button>
          ))}
        </div>
        {pendingColor && (
          <div className="fvp-qty-block" style={{marginTop: 10}}>
            <span className="fvp-qty-label">Quantité :</span>
            <button className="qty-btn" onClick={() => setPendingQty(q => Math.max(1, q - 1))} disabled={pendingQty <= 1}>−</button>
            <span className="qty-num fvp-qty-num">{pendingQty}</span>
            <button className="qty-btn" onClick={() => setPendingQty(q => q + 1)}>+</button>
            <button className="fvp-add-btn" style={{marginLeft: 8}} onClick={handleAdd}>
              ✅ Ajouter {pendingQty} cœur{pendingQty > 1 ? "s" : ""} {HEART_COLORS.find(c=>c.id===pendingColor)?.name}
            </button>
          </div>
        )}
      </div>
...
function Step2_Customize({ productType, config, setConfig }) {
  // flowerVariants = [{flowerID, colorHex, qty}]  replaces config.flowers + config.colors
  const variants = config.flowerVariants || [];
  const totalFlowers = variants.reduce((s, v) => s + v.qty, 0);

  const setVariants = (v) => setConfig(c => ({ ...c, flowerVariants: v }));

  // heartVariants = [{colorID, colorHex, colorName, qty}]
  const heartVariants = config.heartVariants || [];
  const setHeartVariants = (v) => setConfig(c => ({ ...c, heartVariants: v }));
  ...
  useEffect(() => {
    ...
    setConfig((c) => {
      const total = (c.flowerVariants || []).reduce((s, v) => s + v.qty, 0);
      let emb = c.emballage;
  ...
  if (productType === "pot") {
    ...
    const totalHearts = heartVariants.reduce((s, v) => s + v.qty, 0);
    ...
    return (
      <div className="step-content">
        ...
        {(config.potType === "Cœurs" || config.potType === "Mixte") && (
          <>
            <label className="field-label">💗 Cœurs décoratifs
              {totalHearts > 0 && <span style={{marginLeft:8,fontSize:".8rem",color:"var(--rose-deep)",fontWeight:600}}>{totalHearts} cœur{totalHearts>1?"s":""} ajouté{totalHearts>1?"s":""}</span>}
            </label>
            <HeartVariantPicker variants={heartVariants} onChange={setHeartVariants} />
          </>
        )}

        {(config.potType === "Fleurs" || config.potType === "Mixte" || !config.potType) && (
          <>
            <label className="field-label">🌷 Fleurs
              {totalFlowers > 0 && <span style={{marginLeft:8,fontSize:".8rem",color:"var(--rose-deep)",fontWeight:600}}>{totalFlowers} fleur{totalFlowers>1?"s":""} · {fmtPrice(variants.reduce((s,v)=>{const f=FLOWERS.find(fl=>fl.id===v.flowerID);return s+(f?f.price*v.qty:0);},0))}</span>}
            </label>
            <FlowerVariantPicker flowers={FLOWERS} variants={variants} onChange={setVariants} />
          </>
        )}

        <label className="field-label" style={{marginTop:16}}>Fillers <small>(optionnel)</small></label>
  ...
  if (productType === "keychain") {
    return (
      <div className="step-content">
        ...
        <label className="field-label">🌷 Fleurs
          {totalFlowers > 0 && <span style={{marginLeft:8,fontSize:".8rem",color:"var(--rose-deep)",fontWeight:600}}>{totalFlowers} fleur{totalFlowers>1?"s":""}</span>}
        </label>
        <FlowerVariantPicker flowers={KEYCHAINS} variants={variants} onChange={setVariants} />
      </div>
    );
  }
  ...
  return (
    <div className="step-content">
      ...
      <label className="field-label">🌷 Fleurs
        {totalFlowers > 0 && (
          <span style={{marginLeft:8,fontSize:".8rem",color:"var(--rose-deep)",fontWeight:600}}>
            {totalFlowers} fleur{totalFlowers>1?"s":""} · {fmtPrice(variants.reduce((s,v)=>{const f=FLOWERS.find(fl=>fl.id===v.flowerID);return s+(f?f.price*v.qty:0);},0))}
          </span>
        )}
      </label>
      <FlowerVariantPicker flowers={FLOWERS} variants={variants} onChange={setVariants} />
...
function Step3_Delivery({ config, setConfig, savedCustomer }) {
  ...
  const nameHasDigit   = /\d/.test(nameVal);
  const nameHasSpecial = nameVal.length > 0 && /[^a-zA-ZÀ-ÿ\s]/.test(nameVal);
  const nameEmpty      = nameVal.trim().length === 0;

  const addressVal     = (config.address || "").trim();
  const addressOnlyNums = addressVal.length > 0 && !/[a-zA-ZÀ-ÿ]/.test(addressVal);

  const errors = {
    name:    nameEmpty ? "required" : nameHasDigit ? "digits" : nameHasSpecial ? "special" : null,
    phone:   !phone ? "required" : !phoneValid ? "format" : null,
    address: !addressVal ? "required" : addressOnlyNums ? "onlyDigits" : null,
  };

  // Mark as touched immediately on change (real-time) if already touched
  const handleNameChange = (val) => {
    setConfig(c => ({ ...c, name: val }));
    setTouched(t => ({ ...t, name: true }));
  };
  const handleAddressChange = (val) => {
    setConfig(c => ({ ...c, address: val }));
  ...
  return (
    <div className="step-content">
      ...
      />
      {touched.name && errors.name === "required" && <p className="field-err-msg">⚠️ Le nom est obligatoire.</p>}
      {touched.name && errors.name === "digits"   && <p className="field-err-msg">⚠️ Le nom ne peut pas contenir de chiffres.</p>}
      {touched.name && errors.name === "special"  && <p className="field-err-msg">⚠️ Le nom ne peut contenir que des lettres et des espaces.</p>}

      {/* ── Téléphone ── */}
      <input
        className={`field-input${touched.phone && errors.phone ? " field-error" : ""}`}
        type="tel" placeholder="Téléphone * — 10 chiffres ex: 0660606060"
        value={config.phone || ""} maxLength={10}
        onBlur={() => mark("phone")}
      ...
      {touched.phone && errors.phone === "format"   && <p className="field-err-msg">⚠️ Format invalide — 10 chiffres (ex: 0660606060).</p>}

      {/* ── Adresse ── */}
      <input
        className={`field-input${touched.address && errors.address ? " field-error" : ""}`}
        type="text" placeholder="Adresse complète * — ex: Rue 05, Kouba / العنوان الكامل"
        value={config.address || ""}
        onBlur={() => mark("address")}
        onChange={(e) => handleAddressChange(e.target.value)}
      />
      {touched.address && errors.address === "required"   && <p className="field-err-msg">⚠️ L'adresse est obligatoire.</p>}
      {touched.address && errors.address === "onlyDigits" && <p className="field-err-msg">⚠️ L'adresse doit contenir des lettres (ex: "Rue 05" est valide, "05" seul est invalide).</p>}
...
function Step4_Confirm({ productType, config, totalPrice, basket = [], onWhatsApp, onAddToBasket, onBack }) {
  ...
  if (productType === "pot") {
    ...
    // New heartVariants
    (config.heartVariants || []).forEach(v => {
      lines.push({ label: `💗 Cœur ${v.colorName} ×${v.qty}`, price: 0, dot: v.colorHex });
    });
  }

  // New flowerVariants — each variant is its own line
  (config.flowerVariants || []).forEach(v => {
    const f = allFlowers.find(fl => fl.id === v.flowerID);
    const colorName = f?.colors.find(c => c.hex === v.colorHex)?.name || "";
    if (f) lines.push({ label: `${f.emoji} ${f.name} ×${v.qty}`, sub: colorName, price: f.price * v.qty, dot: v.colorHex });
  });
        ...
        <div className="summary-section">
          {lines.map((l, i) => (
            <div key={i} className="summary-item">
              <span>
                {l.dot && <span className="flower-dot" style={{ background: l.dot, border: isLight(l.dot) ? "1px solid #ddd" : "none" }} />}
                {l.label}
...
function buildWhatsAppMessage(productType, config, totalPrice) {
  ...
  if (productType === "pot") {
    ...
    const hvs = config.heartVariants || [];
    if (hvs.length) {
      lines.push("💗 *Cœurs décoratifs :*");
      hvs.forEach(v => lines.push(`  ● ${v.qty}× Cœur ${v.colorName}`));
    }
  }

  // flowerVariants — detailed per variant
  const fvs = config.flowerVariants || [];
  if (fvs.length) {
    lines.push("");
    lines.push("🌷 *Composition florale :*");
    fvs.forEach(v => {
      const f = allFlowers.find(fl => fl.id === v.flowerID);
      const colorName = f?.colors.find(c => c.hex === v.colorHex)?.name || "";
      if (f) lines.push(`  ● ${v.qty}× ${f.emoji} ${f.name} — ${colorName} — ${fmtDA(f.price * v.qty)}`);
    });
    const flowerTotal = fvs.reduce((s, v) => {
      const f = allFlowers.find(fl => fl.id === v.flowerID);
      return s + (f ? f.price * v.qty : 0);
    }, 0);
    lines.push(`  Sous-total fleurs : ${fmtDA(flowerTotal)}`);
  }
...
// ─── BASKET ITEM DETAIL ──────────────────────────────────────────────────────
function BasketItemDetail({ item }) {
  const { productType, config } = item;
  const allFlowers = [...FLOWERS, ...KEYCHAINS.filter(k => !FLOWERS.find(f => f.id === k.id))];
  const isLight = (h) => ["#ffffff","#fff176","#ffd6e0","#ffcba4","#81d4fa","#d8b4e2","#ce93d8","#fafafa"].includes(h);

  const details = [];

  if (productType === "lamp") {
    const m = LAMP_MODELS.find(m => m.id === config.lampModel);
    if (m) details.push({ text: `${m.emoji} Modèle : ${m.name}` });
    const cols = (config.lampColors || []).map(h => LAMP_COLORS.find(c => c.hex === h)?.name).filter(Boolean);
    if (cols.length) details.push({ text: `🎨 Couleurs : ${cols.join(" · ")}` });
    if ((config.extraBatteries || 0) > 0) details.push({ text: `🔋 +${config.extraBatteries} pack(s) piles` });
  }
  if (productType === "board") {
    const bc = BOARD_CONFIGS.find(b => b.id === config.boardConfig);
    if (bc) details.push({ text: `🌸 ${bc.name}` });
    if (config.boardLed) details.push({ text: "✨ LED incluse" });
  }
  if (productType === "pot") {
    if (config.potType) details.push({ text: `🪴 Type : ${config.potType}` });
    (config.heartVariants || []).forEach(v =>
      details.push({ dot: v.colorHex, text: `💗 ${v.qty}× Cœur ${v.colorName}`, light: isLight(v.colorHex) })
    );
  }
  if (productType === "box") details.push({ text: "🎁 Emballage Box" });

  // flowerVariants — one badge per variant
  (config.flowerVariants || []).forEach(v => {
    const f = allFlowers.find(fl => fl.id === v.flowerID);
    const colorName = f?.colors.find(c => c.hex === v.colorHex)?.name || "";
    if (f) details.push({ dot: v.colorHex, light: isLight(v.colorHex), text: `${f.emoji} ${v.qty}× ${f.name} — ${colorName}` });
  });

  const fillers = Object.entries(config.fillerQtys || {}).filter(([,q]) => q > 0);
  fillers.forEach(([id, qty]) => {
    const fl = FILLERS.find(f => f.id === id);
    if (fl) details.push({ text: `${fl.emoji} ${fl.name} ×${qty}` });
  });

  if (productType === "bouquet" && config.emballage) {
    const e = EMBALLAGE_BOUQUET.find(e => e.id === config.emballage);
    if (e) details.push({ text: `🎀 Emballage : ${e.name} (${fmtPrice(e.price)})` });
  }
  if (config.ribbonType) {
    const rt = RIBBON_TYPES.find(r => r.id === config.ribbonType);
    if (rt) details.push({ text: `🎀 Ruban : ${rt.name}${rt.extraCost > 0 ? ` (+${fmtPrice(rt.extraCost)})` : ""}` });
  }
  if (config.perles) details.push({ text: "🪩 Perles décoratives (+350 DA)" });

  if (!details.length) return null;
  return (
    <div className="basket-item-details">
      {details.map((d, i) => (
        <div key={i} className="basket-item-detail-row">
          {d.dot && <span className="fvp-dot" style={{background: d.dot, border: d.light ? "1px solid #ccc" : "none", marginRight: 5, flexShrink: 0}} />}
          {d.text}
        </div>
      ))}
    </div>
...
function BasketView({ basket, onRemoveItem, onContinueShopping, onCheckout, onWhatsAppBasket }) {
  ...
  return (
    <div style={{background:"var(--cream)", minHeight:"100vh"}}>
      ...
      <div className="bottom-bar" style={{flexDirection:"column",gap:8,padding:"12px 16px"}}>
        <button className="back-btn" style={{width:"100%",textAlign:"center"}} onClick={onContinueShopping}>
          ← Continuer mes achats
        </button>
        {basket.length > 0 && (
          <>
            <button className="next-btn" style={{width:"100%",textAlign:"center"}} onClick={onCheckout}>
              ➕ Ajouter un article
            </button>
            <button className="whatsapp-btn" style={{width:"100%"}} onClick={onWhatsAppBasket}>
              <span className="wa-icon">
                <svg viewBox="0 0 24 24" width="20" height="20" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
              </span>
              Commander ce panier via WhatsApp 📲
            </button>
          </>
        )}
      </div>
...
export default function App() {
  ...
  const calcPrice = (cfg = config, pType = productType) => {
    ...
    // flowerVariants: [{flowerID, colorHex, qty}]
    (cfg.flowerVariants || []).forEach(({ flowerID, qty }) => {
      const f = FLOWERS.find(fl => fl.id === flowerID) || KEYCHAINS.find(fl => fl.id === flowerID);
  ...
  const canNext = () => {
    ...
    if (step === 2) {
      ...
      if (productType === "keychain") return (config.flowerVariants || []).some(v => v.qty > 0);
      if (productType === "board")    return !!config.boardConfig;
      if (productType === "pot")      return (config.potType === "Cœurs")
        ? (config.heartVariants || []).some(v => v.qty > 0)
        : (config.flowerVariants || []).some(v => v.qty > 0);
      if (productType === "bouquet") {
        const hasFlowers = (config.flowerVariants || []).some(v => v.qty > 0);
        return hasFlowers && !!config.emballage && !!config.ribbonType;
      }
      return (config.flowerVariants || []).some(v => v.qty > 0);
    }
    if (step === 3) {
      const ph = (config.phone||"").replace(/\s/g,"");
      const phoneOk = /^0[5-7]\d{8}$/.test(ph);
      const nameVal = (config.name || "").trim();
      const nameValid = nameVal.length > 0 && /^[a-zA-ZÀ-ÿ\s]+$/.test(nameVal);
      const addressVal = (config.address || "").trim();
      const addressValid = addressVal.length > 0 && /[a-zA-ZÀ-ÿ]/.test(addressVal);
      return !!(nameValid && phoneOk && addressValid && config.wilaya && config.deliveryType);
    }
    return true;
  };

  // Returns list of missing bouquet fields for the hint bar
  const bouquetMissingFields = () => {
    if (productType !== "bouquet" || step !== 2) return [];
    const missing = [];
    if (!(config.flowerVariants || []).some(v => v.qty > 0)) missing.push("au moins une fleur");
    if (!config.emballage)  missing.push("le type d'emballage");
    if (!config.ribbonType) missing.push("le type de ruban");
    return missing;
  };
  ...
  const buildBasketWA = () => {
    ...
    all.forEach((item, i) => {
      ...
      lines.push(`  💰 Sous-total : ${fmtDA(item.price)}`);
      lines.push("");
    });
  ...
  // Build WA message for basket-only (no current item in progress)
  const buildBasketOnlyWA = () => {
    const fmtDA = (n) => `${n.toLocaleString()} DA`;
    const all = basket;
    const lastConfig = basket.length > 0 ? basket[basket.length - 1].config : {};
    const w = WILAYAS.find(wl => wl.name === lastConfig.wilaya);
    const deliveryCost = w ? (lastConfig.deliveryType === "stopDesk" ? w.stopDesk : w.home) : 0;
    const grand = all.reduce((s, i) => s + i.price, 0);
    let lines = ["🌸 *Nouvelle commande — JardinElle Store*", "━━━━━━━━━━━━━━━━━━━━━", `📦 *${all.length} article${all.length>1?"s":""}*`, ""];
    ...
    lines.push("━━━━━━━━━━━━━━━━━━━━━");
    lines.push(`💰 *TOTAL PANIER : ${fmtDA(grand)}*`);
    lines.push("🕒 Délai de préparation : 3 à 7 jours ouvrables.");
    lines.push("💳 Paiement à la livraison (cash)");
    lines.push("");
    lines.push("👤 *Informations client :*");
    if (lastConfig.name)    lines.push(`  Nom : ${lastConfig.name}`);
  ...
  return (
    <div className="app-shell">
      ...
      {step < 4 && (
        <>
          {step === 2 && productType === "bouquet" && !canNext() && bouquetMissingFields().length > 0 && (
            <div className="bouquet-hint-bar">
              ⚠️ Encore requis : <strong>{bouquetMissingFields().join(", ")}</strong>
            </div>
          )}
...
function buildProductLines(productType, config, totalPrice) {
  ...
  // flowerVariants — one line per variant with color + qty
  const fvs = config.flowerVariants || [];
  if (fvs.length) {
    lines.push("  🌷 Composition florale :");
    fvs.forEach(v => {
...
.fvp-wrapper { display: flex; flex-direction: column; gap: 12px; }
...
.fvp-variant-row {
  display: flex; align-items: center; gap: 6px;
  padding: 5px 0; border-bottom: 1px solid var(--border-lt); flex-wrap: wrap;
}
.fvp-variant-row:last-child { border-bottom: none; }
.fvp-dot {
  width: 14px; height: 14px; border-radius: 50%; flex-shrink: 0; display: inline-block;
}
.fvp-variant-name { flex: 1; font-size: .82rem; color: var(--text); min-width: 120px; }
.fvp-qty-row { display: flex; align-items: center; gap: 4px; }
.fvp-price { font-size: .78rem; color: var(--rose-deep); font-weight: 600; min-width: 60px; text-align: right; }
.fvp-remove {
  background: none; border: none; cursor: pointer; color: #c0392b;
  font-size: .8rem; padding: 2px 4px; border-radius: 4px; opacity: .7;
}
.fvp-remove:hover { opacity: 1; }
...
.fvp-flower-btn {
  ...
  border-radius: var(--r-xs); padding: 8px 4px; cursor: pointer;
  transition: border-color .15s, background .15s;
}
.fvp-flower-btn.selected { border-color: var(--rose); background: var(--rose-pale); }
.fvp-flower-emoji { font-size: 1.4rem; }
.fvp-flower-label { font-size: .72rem; font-weight: 600; color: var(--text); }
.fvp-flower-price { font-size: .65rem; color: var(--rose-deep); font-weight: 500; }

.fvp-color-block { background: var(--parchment); border-radius: var(--r-xs); padding: 10px; }
.fvp-color-label { font-size: .78rem; font-weight: 600; color: var(--text-mid); margin-bottom: 8px; }
.fvp-color-row { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 8px; }
.fvp-color-dot {
  width: 28px; height: 28px; border-radius: 50%; cursor: pointer;
  transition: transform .15s, box-shadow .15s;
}
.fvp-color-dot.active { transform: scale(1.2); box-shadow: 0 0 0 3px var(--rose-deep); }
.fvp-color-name-badge {
  display: inline-block; padding: 3px 10px; border-radius: 99px;
  font-size: .75rem; font-weight: 600; margin-bottom: 8px;
}
.fvp-qty-block { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin-top: 8px; }
.fvp-qty-label { font-size: .8rem; color: var(--text-mid); font-weight: 500; }
.fvp-qty-num { font-size: 1rem; font-weight: 700; color: var(--text); min-width: 24px; text-align: center; }
.fvp-subtotal { font-size: .82rem; color: var(--rose-deep); font-weight: 600; margin-left: 4px; }
.fvp-add-btn {
  width: 100%; margin-top: 10px; padding: 10px 12px; border: none;
  border-radius: var(--r-sm); background: linear-gradient(135deg, var(--sage), #5d8a62);
  color: #fff; font-size: .85rem; font-weight: 600; cursor: pointer;
  font-family: var(--font-body); transition: opacity .15s;
}
.fvp-add-btn:hover { opacity: .9; }
...
.basket-item-detail-row {
  display: flex; align-items: center; gap: 5px;
  font-size: .78rem; color: var(--text-mid); padding: 2px 0; line-height: 1.5;
}

/* ═══ BOUQUET VALIDATION HINT ══════════════════════════════════════ */
.bouquet-hint-bar {
  background: #fff3f3; border-top: 1.5px solid #f8b8b8;
  color: #c0392b; font-size: .8rem; text-align: center;

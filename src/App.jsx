...
function Step3_Delivery({ config, setConfig, savedCustomer }) {
  ...
  const nameHasDigit = /\d/.test(config.name || "");
  const nameHasSpecial = /[^a-zA-ZÀ-ÿ\s]/.test(config.name || "");

  const addressVal = (config.address || "").trim();
  const addressHasLetters = /[a-zA-ZÀ-ÿ]/.test(addressVal);
  const addressValid = addressVal.length > 0 && addressHasLetters;

  const errors = {
    name:    nameHasDigit ? "digits" : nameHasSpecial ? "special" : !nameVal ? "required" : null,
    phone:   !phone ? "required" : (!phoneValid ? "format" : null),
    address: !addressVal ? "required" : !addressHasLetters ? "onlyDigits" : null,
  };
  const mark = (k) => setTouched(t => ({ ...t, [k]: true }));

  // Real-time: mark as touched on change if already touched
  ...
  return (
    <div className="step-content">
      ...
      {touched.address && errors.address === "onlyDigits" && <p className="field-err-msg">⚠️ L'adresse doit contenir au moins quelques lettres (ex: "Rue 05" est valide, "05" seul est invalide).</p>}
    ...
    <div className="step-content">
      ...
      <div className="summary-card">
        ...
        {config.wilaya && (
          <div className="summary-section">
            ...
            <div className="summary-item">
              ...
              <span style={{color:"var(--sage)",fontWeight:600}}>{fmtPrice(deliveryCost)}</span>
            </div>
            {basket.length > 0 && (
              <div style={{fontSize:".75rem",color:"var(--text-lt)",marginTop:4,fontStyle:"italic"}}>
                ✅ Frais de livraison uniques pour tout le panier
              </div>
            )}
          </div>
        )}
        ...
        <div className="summary-total">
          <span>{basket.length > 0 ? "Total panier complet" : "Total"}</span>
          <span className="total-val">{fmtPrice(grandTotal)}</span>
        </div>
      ...
      {basket.length > 0 && (
        <div className="basket-preview">
          ...
          {deliveryCost > 0 && (
            <div className="basket-preview-item" style={{color:"var(--sage)"}}>
              <span>🚚 Livraison (×1)</span><span>{fmtPrice(deliveryCost)}</span>
            </div>
          )}
...
function BasketItemDetail({ item }) {
  ...
  if (productType === "lamp") {
    const m = LAMP_MODELS.find(m => m.id === config.lampModel);
    if (m) details.push(`${m.emoji} Modèle : ${m.name}`);
    const cols = (config.lampColors || []).map(h => LAMP_COLORS.find(c => c.hex === h)?.name).filter(Boolean);
    if (cols.length) details.push(`🎨 Couleurs : ${cols.join(" · ")}`);
    if ((config.extraBatteries || 0) > 0) details.push(`🔋 +${config.extraBatteries} pack(s) piles`);
  }
  if (productType === "board") {
    const bc = BOARD_CONFIGS.find(b => b.id === config.boardConfig);
    if (bc) details.push(`🌸 ${bc.name}`);
    if (config.boardLed) details.push("✨ LED incluse");
  }
  if (productType === "pot") {
    if (config.potType) details.push(`🪴 Type : ${config.potType}`);
    const hearts = (config.heartColors || []).map(id => HEART_COLORS.find(c => c.id === id)?.name).filter(Boolean);
    if (hearts.length) details.push(`💗 Cœurs : ${hearts.join(", ")}`);
  }
  if (productType === "box") details.push("🎁 Emballage Box");

  const flowerList = Object.entries(config.flowers || {}).filter(([, q]) => q > 0);
  if (flowerList.length) {
    const flStr = flowerList.map(([id, qty]) => {
      const f = allFlowers.find(fl => fl.id === id);
      const colorHex = (config.colors || {})[id] || f?.colors[0].hex;
      const colorName = f?.colors.find(c => c.hex === colorHex)?.name || "";
      return f ? `${f.emoji} ${f.name} ×${qty}${colorName ? ` (${colorName})` : ""}` : null;
    }).filter(Boolean);
    details.push(`🌷 Fleurs : ${flStr.join(", ")}`);
  }

  const fillers = Object.entries(config.fillerQtys || {}).filter(([,q]) => q > 0);
  if (fillers.length) {
    const filStr = fillers.map(([id, qty]) => {
      const fl = FILLERS.find(f => f.id === id);
      return fl ? `${fl.emoji} ${fl.name} ×${qty}` : null;
    }).filter(Boolean);
    details.push(`🌿 Fillers : ${filStr.join(", ")}`);
  }

  if (productType === "bouquet" && config.emballage) {
    const e = EMBALLAGE_BOUQUET.find(e => e.id === config.emballage);
    if (e) details.push(`🎀 Emballage : ${e.name} — ${fmtPrice(e.price)}`);
  }
  if (config.ribbonType) {
    const rt = RIBBON_TYPES.find(r => r.id === config.ribbonType);
    if (rt) details.push(`🎀 Ruban : ${rt.name}${rt.extraCost > 0 ? ` (+${fmtPrice(rt.extraCost)})` : ""}`);
  }
  if (config.perles) details.push("🪩 Perles décoratives (+350 DA)");

  if (!details.length) return null;
  return (
    <div className="basket-item-details">
      {details.map((d, i) => <div key={i} className="basket-item-detail-row">{d}</div>)}
    </div>
  );
}
...
export default function App() {
  ...
  const calcPrice = (cfg = config, pType = productType) => {
    ...
    // ⚠️ La livraison N'est PAS incluse ici — elle est unique pour tout le panier
    return total;
  };

  // Frais de livraison : une seule fois pour tout le panier
  const calcDelivery = (cfg = config) => {
  ...
  // Prix de l'article en cours (sans livraison)
  const totalPrice = calcPrice();
  // Total panier = somme des articles du panier + article en cours + livraison (x1)
  const basketProductsTotal = basket.reduce((s, i) => s + i.price, 0);
  const deliveryCost = calcDelivery();
  const grandTotal = basketProductsTotal + totalPrice + deliveryCost;
  const STEPS = 4;
  ...
  const canNext = () => {
    ...
    if (step === 2) {
      ...
      // Bouquet & Box: flowers required. Bouquet also needs emballage + ribbon.
      if (productType === "bouquet") {
        const hasFlowers = Object.values(config.flowers || {}).some((q) => q > 0);
    ...
    if (step === 3) {
      ...
      const nameValid = /^[a-zA-ZÀ-ÿ\s]+$/.test((config.name || "").trim()) && (config.name || "").trim().length > 0;
      const addressVal = (config.address || "").trim();
  ...
  // Missing bouquet fields helper (used for inline hint)
  const bouquetMissingFields = () => {
    if (productType !== "bouquet" || step !== 2) return [];
    const missing = [];
    if (!Object.values(config.flowers || {}).some((q) => q > 0)) missing.push("au moins une fleur");
  ...
  const buildBasketWA = () => {
    const fmtDA = (n) => `${n.toLocaleString()} DA`;
    // totalPrice est déjà SANS livraison grâce à calcPrice()
    ...
    // Somme produits (tous sans livraison) + livraison x1
    const productsSum = all.reduce((s, i) => s + i.price, 0);
    const grand = productsSum + delivery;
    ...
    all.forEach((item, i) => {
      ...
      lines.push(`  💰 Sous-total article : ${fmtDA(item.price)}`);
      lines.push("");
    });
    ...
    if (lastConfig.wilaya) {
      ...
      lines.push("━━━━━━━━━━━━━━━━━━━━━");
      lines.push("🚚 *Livraison (×1 pour tout le panier) :*");
      lines.push(`  📍 ${lastConfig.wilaya} · ${mode} — ${fmtDA(delivery)}`);
      lines.push("");
    }

    lines.push("━━━━━━━━━━━━━━━━━━━━━");
    lines.push(`🛒 Produits : ${fmtDA(productsSum)}`);
    if (delivery > 0) lines.push(`🚚 Livraison : ${fmtDA(delivery)}`);
    lines.push(`💰 *TOTAL À PAYER : ${fmtDA(grand)}*`);
    ...
    <div className="app-shell">
      ...
      <header className="app-header" ref={topRef}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <PriceTag price={step === 4 ? grandTotal : totalPrice} />
        </div>
      </header>
      ...
      <main className="app-main">
        ...
        {step === 4 && <Step4_Confirm
          productType={productType} config={config}
      ...
      {step < 4 && (
        <>
          {step === 2 && productType === "bouquet" && !canNext() && (
